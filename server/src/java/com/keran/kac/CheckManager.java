...
public class CheckManager {
    ...
    /**
     * 处理一次违规：累加 vl、冷却、告警、处罚。
     * 若在异步线程被调用(如聊天事件), 自动切到主线程执行以保证安全。
     */
    public void handleFlag(PlayerData data, CheckType type, String info, int addVl) {
        if (!Bukkit.isPrimaryThread()) {
            Bukkit.getScheduler().runTask(plugin, () -> handleFlagSync(data, type, info, addVl));
            return;
        }
        handleFlagSync(data, type, info, addVl);
    }

    private void handleFlagSync(PlayerData data, CheckType type, String info, int addVl) {
        ...
        data.addVl(type, addVl);
        data.addFlag(type);
        totalFlags.merge(type, 1, Integer::sum);

        int vl = data.getVl(type);
        ConfigManager cfg = plugin.getConfigManager();
        ...
        plugin.getLogger().warning(String.format(
                "[KAC-FLAG] %s 触发 %s(%s) vl=%d info=%s loc=%s",
                data.getName(), type.getDisplayName(), type.getId(), vl, info,
                formatLoc(player)));

        if (notify) {
            broadcastAlert(data, type, vl, info, player.getLocation().getWorld() == null ? "?" : player.getLocation().getWorld().getName());
        }
        ...
        if (banVl > 0 && vl >= banVl) {
            ...
            player.kickPlayer(ChatColor.translateAlternateColorCodes('&',
                    cfg.getBanMessage().replace("%reason%", type.getDisplayName() + " (vl=" + vl + ")")));
        } else if (kickVl > 0 && vl >= kickVl) {
            data.resetVl(type);
            data.addPunishment(type);
            player.kickPlayer(ChatColor.translateAlternateColorCodes('&',
                    cfg.getKickMessage().replace("%reason%", type.getDisplayName() + " (vl=" + vl + ")")));
        }
    }

    /** 向所有有 kac.notify 权限且开启告警的玩家广播 */
    private void broadcastAlert(PlayerData data, CheckType type, int vl, String info, String world) {
        String msg = ChatColor.translateAlternateColorCodes('&',
                String.format("§8[§cKAC§8] §e%s §7触发 §c%s§8(§7vl:%d§8)§7 [%s] %s",
