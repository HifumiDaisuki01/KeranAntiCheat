...
public class ConfigManager {
     ...
     * 踢出阈值。
     *
     * <p><b>v3.1 (KAC-09)</b>：v3.0 只在配置值 {@code < 0} 时回落到全局默认，
     * 但 config.yml 里 28 个检测器<b>全部显式写着 {@code kick-vl: 0}</b>，
     * 于是 {@code default-kick-vl: 20} 永远不会生效——"默认处罚阈值"形同虚设，
     * 任何作弊者都不会被自动踢出，反作弊退化成纯告警器。
     *
     * <p>v3.1 语义修正：
     * <ul>
     *   <li>{@code kick-vl} 缺失或 {@code 0} → 回落到 {@code punishments.default-kick-vl}；</li>
     *   <li>{@code kick-vl: -1} → <b>显式关闭</b>该检测的自动踢出（用于纯观察模式）。</li>
     * </ul>
     */
    public int getKickVl(CheckType t) {
        int v = config.getInt("checks." + t.getId() + ".kick-vl", 0);
        if (v == 0) {
            return defaultKickVl;
        }
        return v < 0 ? 0 : v;
    }

    /**
     * 封禁阈值。语义同 {@link #getKickVl(CheckType)}：
     * {@code 0}/缺失 → 回落到全局默认；{@code -1} → 显式关闭自动封禁。
     */
    public int getBanVl(CheckType t) {
        int v = config.getInt("checks." + t.getId() + ".ban-vl", 0);
        if (v == 0) {
            return defaultBanVl;
        }
        return v < 0 ? 0 : v;
