...
/**
 * /kac 命令系统与 Tab 补全。
 * 整合: 反作弊管理(kac 原命令) + 客户端桥接(cinfo/detail/modsmd5/packsmd5/list)。
 */
public class KacCommand implements CommandExecutor, TabCompleter {
    ...
    private final ClientBridgeCommands clientCommands;

    public KacCommand(KeranAntiCheat plugin, ClientBridgeCommands clientCommands) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "verbose":
                ...
                break;
            default:
                // 客户端桥接子命令: cinfo / detail / modsmd5 / packsmd5 / list
                if (clientCommands != null && clientCommands.handle(sender, args)) {
                    break;
                }
    ...
    private void help(CommandSender sender) {
        sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                "§8===== §cKeran Anti Cheat §8v" + plugin.getDescription().getVersion() + " §8=====\n" +
                        PREFIX + "§f/kac help §7- 帮助\n" +
                        PREFIX + "§f/kac alerts [on|off] §7- 切换告警接收\n" +
                        PREFIX + "§f/kac info <玩家> §7- 查看违规记录\n" +
                        PREFIX + "§f/kac cinfo <玩家> §7- 查看客户端指纹\n" +
                        PREFIX + "§f/kac detail <玩家> §7- 客户端详细 mods/材质包清单\n" +
                        PREFIX + "§f/kac modsmd5 <玩家> §7- 客户端 mods 目录 MD5\n" +
                        PREFIX + "§f/kac packsmd5 <玩家> §7- 客户端材质包目录 MD5\n" +
                        PREFIX + "§f/kac list §7- 已上报客户端玩家列表\n" +
    ...
    private static final List<String> SUB_COMMANDS = new ArrayList<>(Arrays.asList(
            "help", "reload", "alerts", "info", "reset", "checks", "toggle",
            "ban", "unban", "kick", "stats", "verbose"));
    static {
        SUB_COMMANDS.addAll(ClientBridgeCommands.SUB_COMMANDS);
    }
